...
/**
 * Sample Savona cruise calls for booking date/ship selection.
 * Replace with imported schedule feeds when production data is connected.
 */
const SAMPLE_SAVONA_SCHEDULE: readonly CruiseScheduleEntry[] = [
  {
    date: "2026-09-12",
    ship: "MSC Seaside",
    arrival: "07:00",
...
export function loadAllSavonaScheduleEntries(): CruiseScheduleEntry[] {
  return [...SAMPLE_SAVONA_SCHEDULE];
}

export function getSavonaShipsOnDate(
  isoDate: string,
  entries: readonly CruiseScheduleEntry[] = loadAllSavonaScheduleEntries(),
): BookingShipVisit[] {
