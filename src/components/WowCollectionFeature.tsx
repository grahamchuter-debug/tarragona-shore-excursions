...
import { WOW_COLLECTION_PATH, wowCollection } from "@/data/wow-collection";
      ...
      <div className="container-wide">
        <div className="card-signature grid gap-0 overflow-hidden border-white/10 bg-white/5 lg:grid-cols-2">
          ...
          <div className="flex flex-col justify-center bg-[#faf8f5] p-8 text-gray-900 sm:p-10 lg:p-12">
            <p className="section-eyebrow text-coastal-700">Future premium shared experiences</p>
            ...
            <div className="mt-6 grid gap-2 sm:grid-cols-2">
              {wowCollection.plannedExperiences.map((item) => (
                <div key={item.title} className="flex items-start gap-2 text-sm text-gray-700">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={WOW_COLLECTION_PATH} className="btn-accent">
                Explore the Future Premium Collection →
              </Link>
              <Link href={SIGNATURE_EXPERIENCE_PATH} className="btn-secondary">
                Future Premium Roman Discovery
              </Link>
            </div>
          </div>
        </div>
